  ...
  var processUrl = function(url, callback) {
    ...
    request(req, function(error, response, body) {
      ...
      $('meta[http-equiv="X-UA-Compatible"]').each(function() {
          ...
          result.metaxuacompatible = $(this).attr("content");
      ...
      if(response.headers["X-UA-Compatible"]) {
        ...
        result.hdrxuacompatible = response.headers["X-UA-Compatible"];

        if(response.headers["X-UA-Compatible"].indexOf("chrome") > -1) {
          result.cf = true;
